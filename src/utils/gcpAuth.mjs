...
export const generateIdToken = async (credentials, targetAudience, log) => {
  ...
  if (typeof log?.info !== 'function') {
    ...
    console.error(
      { credentials: !!credentials, targetAudience, log },
      'generateIdToken initial check'
    );
  ...
  try {
    ...
    log.debug(
      { tokenValue: token, tokenType: typeof token },
      'Token value received from fetchIdToken.'
    ); // What is the token?
    ...
    log.debug(
      ...
      'Token is truthy, attempting final log.'
    );
    // --- END ADDED DEBUG LOGGING ---

    // This is the log that was missing
    log.info(
      { targetAudience, tokenLength: token?.length },
