...
function processSummaryResult(summaryResult, log) {
  ...
  if (typeof summaryResult.currentPowerW === 'number') {
    summaryParts.push(
      `Currently generating ${formatPower(summaryResult.currentPowerW)}.`
    );
  } else {
    log.warn(
      { summaryResult },
      'currentPowerW missing or not a number in summary response.'
    );
  ...
  if (typeof summaryResult.dailyProductionKWh === 'number') {
    ...
    if (energykWh > 0) {
      summaryParts.push(
        `Today's production is ${formatEnergy(energykWh)} so far.`
      );
    } else {
    ...
    log.warn(
      { summaryResult },
      'dailyProductionKWh missing or not a number in summary response.'
    );
  ...
  if (typeof summaryResult.isOnline === 'boolean') {
    ...
    log.warn(
      { summaryResult },
      'isOnline missing or not a boolean in summary response.'
    );
 ...
 */
export const handleGetSummaryIntent = async (
  event,
  log,
  gcpClient,
  handlerConfig
  ...
  try {
    ...
    if (!summaryResult) {
      ...
      const { summaryParts, errorsEncountered: processingErrors } =
        processSummaryResult(summaryResult, log);
      ...
      if (overallErrorsEncountered && summaryParts.length > 0) {
        ...
        speechText =
          "Sorry, I couldn't parse any summary information from the system's response.";
      }
    ...
    log.error(
      { err: error },
      `Error fetching system summary from backend for ${INTENT_NAME}.`
    );
  ...
  log.info(
    { speechText, errorsEncountered: overallErrorsEncountered },
