...
const createMockEvent = (requestType, intentName = null, slots = {}) => ({
    ...
    request: {
        ...
        locale: 'en-US',
        ...(requestType === 'IntentRequest' && {intent: {name: intentName, confirmationStatus: 'NONE', slots}}),
...
describe('Lambda Handler Integration Tests', () => {
    ...
    describe('LaunchRequest Handling', () => {
        it('should return the welcome message', async () => { // Expect correct routing
            ...
            expect(mockGetSecret).toHaveBeenCalledWith(FAKE_SECRET_ID, expect.anything());
            expect(mockGenerateIdToken).toHaveBeenCalledWith(MOCK_GCP_CREDENTIALS, FAKE_AUDIENCE, expect.anything());
            expect(mockCallGcpFunction).not.toHaveBeenCalled();

            // Expect intended behavior
            expect(result.response.outputSpeech.text).toContain('Welcome to Solar Monitor');
    ...
    describe('GetCurrentPowerIntent Handling', () => {
        it('should call GCP and return power', async () => { // Expect correct routing
            ...
            expect(mockCallGcpFunction).toHaveBeenCalledWith(
                FAKE_AUDIENCE, MOCK_ID_TOKEN, expect.objectContaining({dataType: 'current'}), expect.anything() // Use expect.anything() for logger
            );
            // *** ------------------------ ***

            // Expect intended behavior
            expect(result.response.outputSpeech.text).toBe('Your current solar production is 1.6 kilowatts.');
            expect(result.response.shouldEndSession).toBe(true);
        });

        it('should return an error message if callGcpFunction fails', async () => { // Expect correct routing
            const event = createMockEvent('IntentRequest', 'GetCurrentPowerIntent');
            ...
            expect(mockCallGcpFunction).toHaveBeenCalledTimes(1);
            // *** ------------------------ ***
            expect(result.response.outputSpeech.text).toContain("couldn't connect to the solar monitor");
            // Cannot easily assert console logs without mocking, so removed logger assertion
        });

        it('should return an error message if GCP response is missing data', async () => { // Expect correct routing
            const event = createMockEvent('IntentRequest', 'GetCurrentPowerIntent');
            ...
            expect(mockCallGcpFunction).toHaveBeenCalledTimes(1);
            // *** ------------------------ ***
            expect(result.response.outputSpeech.text).toContain("received an incomplete response");
    ...
    describe('GetSummaryIntent Handling', () => {
        ...
        it('should call GCP and return summary', async () => {
            ...
            mockCallGcpFunction.mockResolvedValue({currentPowerW: 2100, dailyProductionKWh: 8.3, isOnline: true});
            const result = await handler(event, MOCK_CONTEXT);
            // *** Expect 1 call again ***
            expect(mockCallGcpFunction).toHaveBeenCalledTimes(1);
            expect(mockCallGcpFunction).toHaveBeenCalledWith(
                FAKE_AUDIENCE, MOCK_ID_TOKEN, expect.objectContaining({dataType: 'summary'}), expect.anything()
            );
            // *** ------------------------ ***
            expect(result.response.outputSpeech.text).toContain('system is online');
            expect(result.response.outputSpeech.text).toContain('generating 2.1 kilowatts');
        });

        it('should return partial summary', async () => {
            const event = createMockEvent('IntentRequest', 'GetSummaryIntent');
            mockCallGcpFunction.mockResolvedValue({dailyProductionKWh: 9.1, isOnline: true});
            const result = await handler(event, MOCK_CONTEXT);
            // *** Expect 1 call again ***
            expect(mockCallGcpFunction).toHaveBeenCalledTimes(1);
            // *** ------------------------ ***
        ...
        it('should return error if call would fail', async () => {
            ...
            expect(mockCallGcpFunction).toHaveBeenCalledTimes(1);
            // *** ------------------------ ***
            expect(result.response.outputSpeech.text).toContain("solar system's reporting service");
        });
    });
    ...
    describe('GetDailyProductionIntent Handling', () => {
        it('should call GCP and return daily production', async () => {
            ...
            expect(result.response.outputSpeech.text).toBe('Your total solar production for the day is 7.8 kilowatt hours.');
            expect(result.response.shouldEndSession).toBe(true);
        });
        ...
        it('should return an error message if callGcpFunction fails', async () => {
            ...
            expect(result.response.outputSpeech.text).toContain("couldn't connect to the solar monitor"); // Adjust if your daily handler has a different error message
            expect(result.response.shouldEndSession).toBe(true);
        });
        ...
        it('should return an error message if GCP response is missing data', async () => {
            ...
            expect(result.response.outputSpeech.text).toContain("received an incomplete response"); // Adjust if your daily handler has a different error message
            expect(result.response.shouldEndSession).toBe(true);
        });
    });
    ...
    describe('GetOnlineStatusIntent Handling', () => {
        it('should call GCP and return online status when true', async () => {
            ...
            expect(result.response.outputSpeech.text).toBe('The solar energy system is currently online and reporting data.');
            expect(result.response.shouldEndSession).toBe(true);
        });
        ...
        it('should call GCP and return offline status when false', async () => {
            ...
            // Check response content
            expect(result.response.outputSpeech.text).toBe('The solar energy system is currently reporting as offline.');
            expect(result.response.shouldEndSession).toBe(true);
        });
        ...
        it('should return an error message if callGcpFunction fails', async () => {
            ...
            expect(result.response.outputSpeech.text).toContain("couldn't retrieve the system status"); // Adjust if your status handler has a different error message
            expect(result.response.shouldEndSession).toBe(true);
        });
        ...
        it('should return an error message if GCP response is missing data', async () => {
            ...
            expect(result.response.outputSpeech.text).toContain("unexpected status format");
            expect(result.response.shouldEndSession).toBe(true);
        });
    });
    ...
    describe('Stop/Cancel/Help/Fallback Handling', () => {
        ...
        it('should handle unknown intent (routed to fallback) correctly', async () => {
            ...
            expect(result.response.outputSpeech.text).toBe("Sorry, I didn't understand that request. You can ask about current power, daily production, or system status. You can also say 'help' for more options. What would you like to know?");
        ...
        it('should handle AMAZON.HelpIntent correctly', async () => {
            ...
            expect(result.response.outputSpeech.text).toContain("You can ask me about your solar energy system. For example, try saying:\n" +
                "        'What's my current power production?',\n" +
                "        'How much energy did I produce today?',\n" +
                "        Or, 'Is the system online?'.\n" +
                "        What would you like to know?"); // Check main part of help text
            expect(result.response.reprompt.outputSpeech.text).toContain("You can ask about current power, daily production, or the system status. What data are you interested in?"); // Check reprompt
    ...
    describe('Auth/Config Error Handling (by index.mjs)', () => {
        ...
        it('should return config error if GCP_SECRET_ID is missing', async () => {
            ...
            const event = createMockEvent('LaunchRequest');
            const result = await handler(event, MOCK_CONTEXT);
            expect(result.response.outputSpeech.text).toContain('not configured correctly');
        ...
        it('should return generic error if getSecret fails', async () => {
            ...
            const event = createMockEvent('LaunchRequest');
            const result = await handler(event, MOCK_CONTEXT);
            expect(result.response.outputSpeech.text).toContain("couldn't retrieve the necessary credentials");
        ...
        it('should return generic error if generateIdToken fails', async () => {
            ...
            const event = createMockEvent('LaunchRequest');
            const result = await handler(event, MOCK_CONTEXT);
            expect(result.response.outputSpeech.text).toContain("encountered an issue authenticating");
