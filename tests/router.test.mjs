...
jest.unstable_mockModule(
  ...
  () => ({ handleGetOnlineStatusIntent: mockGetOnlineStatusHandler })
);
jest.unstable_mockModule(
  '../src/intentHandlers/getSummaryIntentHandler.mjs',
  () => ({ handleGetSummaryIntent: mockGetSummaryHandler })
);
jest.unstable_mockModule(
...
jest.unstable_mockModule(
  ...
  () => ({ handleStopCancelIntent: mockStopCancelHandler })
);
jest.unstable_mockModule(
...
describe('Request Router', () => {
  ...
  const intentRoutingTestCases = [
    {
      intentName: 'GetCurrentPowerIntent',
      expectedHandler: mockCurrentPowerHandler,
    },
    {
      intentName: 'GetDailyProductionIntent',
      expectedHandler: mockDailyProductionHandler,
    },
    {
      intentName: 'GetOnlineStatusIntent',
      expectedHandler: mockGetOnlineStatusHandler,
    },
    ...
    { intentName: 'AMAZON.StopIntent', expectedHandler: mockStopCancelHandler },
    {
      ...
      expectedHandler: mockStopCancelHandler,
    },
    {
      intentName: 'AMAZON.FallbackIntent',
      expectedHandler: mockFallbackHandler,
    },
    ...
    ({ intentName, expectedHandler }) => {
      ...
      if (intentName === 'UnknownIntent') {
        expect(mockLogger.warn).toHaveBeenCalledWith(
          { intentName },
          'Unknown intent name encountered.'
        );
      } else if (
        ...
        expect(mockLogger.info).toHaveBeenCalledWith(
          'Matched AMAZON.Stop/CancelIntent'
        );
      } else {
  ...
  it('should return null for an unknown request type', () => {
    ...
    expect(mockLogger.warn).toHaveBeenCalledWith(
      { requestType: 'UnknownRequestType' },
      'No matching route found, returning null.'
    ); // Changed message
    // *** ------------------ ***
  });
  ...
  it('should return null if event or request structure is invalid/missing', () => {
    ...
    expect(mockLogger.info).toHaveBeenCalledWith(
      '--- Router received event ---'
    ); // Called even with null event
    expect(mockLogger.error).toHaveBeenCalledWith(
      'Router Error: Event missing request type.'
    );
    // *** ------------------ ***
  ...
  it('should route to fallback if IntentRequest is missing intent name', () => {
    const event = {
      request: { type: 'IntentRequest', requestId: 'req-missing-intent' },
    }; // Missing intent obj/name
    const handler = routeRequest(event, mockLogger);
    ...
    expect(mockLogger.error).toHaveBeenCalledWith(
      'Router Error: IntentRequest missing intent name.'
    );
    // *** ------------------ ***
  });
});
