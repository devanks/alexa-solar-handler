...
describe('Request Router', () => {
  ...
  it('should return the LaunchRequest handler for LaunchRequest type', () => {
    ...
    expect(mockLoggerInstance.info).toHaveBeenCalledWith(
      expect.objectContaining({ requestType: 'LaunchRequest' }),
      'Routing request'
    );
    expect(mockLoggerInstance.info).toHaveBeenCalledWith(
      'Routing to LaunchRequest handler.'
    );
  });
  ...
  it('should return the GetCurrentPowerIntent handler for IntentRequest with that name', () => {
    ...
    expect(mockLoggerInstance.info).toHaveBeenCalledWith(
      expect.objectContaining({ intentName: 'GetCurrentPowerIntent' }),
      'Routing IntentRequest.'
    );
    expect(mockLoggerInstance.info).toHaveBeenCalledWith(
      'Routing to GetCurrentPowerIntent handler.'
    );
  });
  ...
  it('should return the GetDailyProductionIntent handler for IntentRequest with that name', () => {
    ...
    expect(mockLoggerInstance.info).toHaveBeenCalledWith(
      expect.objectContaining({ intentName: 'GetDailyProductionIntent' }),
      'Routing IntentRequest.'
    );
    expect(mockLoggerInstance.info).toHaveBeenCalledWith(
      'Routing to GetDailyProductionIntent handler.'
    );
  });
  ...
  it('should return null for IntentRequest with an unknown intent name', () => {
    ...
    expect(mockLoggerInstance.warn).toHaveBeenCalledWith(
      ...
      'No specific handler found for this intent name. Routing to fallback/null.'
    );
  });
  ...
  it('should return null for SessionEndedRequest type', () => {
    ...
    expect(mockLoggerInstance.info).toHaveBeenCalledWith(
      expect.objectContaining({ requestType: 'SessionEndedRequest' }),
      'Routing request'
    );
    expect(mockLoggerInstance.info).toHaveBeenCalledWith(
      'Routing to SessionEndedRequest handler.'
    );
  });
  ...
  it('should return null for an unknown request type', () => {
    ...
    expect(handler).toBeNull();
    expect(mockLoggerInstance.info).toHaveBeenCalledWith(
      expect.objectContaining({ requestType: 'AudioPlayer.PlaybackStarted' }),
      'Routing request'
    );
    expect(mockLoggerInstance.warn).toHaveBeenCalledWith(
      { requestType: 'AudioPlayer.PlaybackStarted' },
      'Received unknown request type. No handler available.'
    );
  });
