...
describe('GetOnlineStatusIntent Handler', () => {
  ...
  test.each([
    ...
    'should return correct status when system is %s',
    async (mockStatusResult, expectedSpeech, _description) => {
      // Arrange
      mockGcpClient.mockResolvedValue(mockStatusResult);
      const expectedResponse = buildTellResponse(expectedSpeech);

      // Act
      const result = await handleGetOnlineStatusIntent(
        mockEvent,
        mockLogger,
        mockGcpClient,
        mockConfig
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        'Handling GetOnlineStatusIntent.'
      );
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        'Calling gcpClient.getSystemStatus...'
      );
      expect(mockLogger.debug).toHaveBeenCalledWith(
        { statusResult: mockStatusResult },
        'Received response from gcpClient.getSystemStatus.'
      );
  ...
  it('should return an error message if the backend call fails', async () => {
    ...
    const expectedSpeech =
      "Sorry, I couldn't retrieve the system status right now. There might be a connection issue. Please try again later.";
    const expectedResponse = buildTellResponse(expectedSpeech);
  ...
  describe('Handling Unexpected Backend Responses', () => {
    test.each(unexpectedResponses)(
      'should return an error message for %s',
      async (mockStatusResult, _description) => {
        // Arrange
        mockGcpClient.mockResolvedValue(mockStatusResult);
        const expectedSpeech =
          "Sorry, I received an unexpected status format from the system. I can't determine if it's online right now.";
        const expectedResponse = buildTellResponse(expectedSpeech);
