...
beforeAll(async () => {
  ...
  const responseBuilderModule = await import(
...
describe('GetSummaryIntent Handler (Single API Call)', () => {
  ...
  test('should return full summary when API succeeds with all data', async () => {
    ...
    // Act
    const result = await handleGetSummaryIntent(
      mockEvent,
      mockLogger,
      mockGcpClient,
      mockHandlerConfig
    ...
    expect(mockLogger.debug.mock.calls[0]).toEqual([
      'Calling GCP function for summary data.',
    ]);
    expect(mockLogger.debug.mock.calls[1]).toEqual([
      { summaryResult: mockApiResponse },
      'Received response from gcpClient.getSystemSummary.',
    ]);
  ...
  describe('Handling Partial or Invalid Data in API Response', () => {
    const partialDataCases = [
      {
        ...
        input: { dailyProductionKWh: 5.5, isOnline: true },
        expectedWarnMsg: 'currentPowerW missing or not a number',
      },
      {
        description: 'null power',
        input: { currentPowerW: null, dailyProductionKWh: 5.5, isOnline: true },
        expectedWarnMsg: 'currentPowerW missing or not a number',
      },
      {
        description: 'missing production',
        input: { currentPowerW: 1500, isOnline: true },
        expectedWarnMsg: 'dailyProductionKWh missing or not a number',
      },
      {
        ...
        expectedWarnMsg: 'isOnline missing or not a boolean',
      },
      {
        description: 'missing power and production',
    ...
    test.each(partialDataCases)(
      'should return partial summary when $description',
      async ({ input, expectedWarnMsg }) => {
        // Arrange
        ...
        if (typeof input.currentPowerW === 'number') {
          expectedParts.push(
            `Currently generating ${input.currentPowerW} formatted W.`
          );
        } else {
          expectedParts.push(
            "Couldn't determine the current power generation."
          );
          errors = true;
        }
        if (typeof input.dailyProductionKWh === 'number') {
          if (input.dailyProductionKWh > 0) {
            expectedParts.push(
              `Today's production is ${input.dailyProductionKWh} formatted kWh so far.`
            );
          } else {
            expectedParts.push(
              'There has been no production recorded yet today.'
            );
          }
        } else {
        ...
        if (typeof input.isOnline === 'boolean') {
          expectedParts.push(
            input.isOnline
              ...
              : 'The system is reporting as offline.'
          );
        } else {
        ...
        const expectedSpeech =
          (errors ? "Here's a partial summary: " : '') +
          expectedParts.join(' ');
        ...
        expect(mockLogger.warn).toHaveBeenCalledWith(
          expect.any(Object),
          expect.stringContaining(expectedWarnMsg)
        );
        expect(result.response.outputSpeech.text).toBe(expectedSpeech);
        expect(result.response.shouldEndSession).toBe(true);
        expect(mockLogger.error).not.toHaveBeenCalled();
        expect(mockLogger.info).toHaveBeenCalledWith(
          expect.objectContaining({ errorsEncountered: true }),
          'Final summary speech constructed.'
        );

        // Check formatters were called ONLY if data was valid
        if (typeof input.currentPowerW === 'number')
          expect(mockFormatPower).toHaveBeenCalledWith(input.currentPowerW);
        else expect(mockFormatPower).not.toHaveBeenCalled();
        if (
          typeof input.dailyProductionKWh === 'number' &&
          input.dailyProductionKWh > 0
        )
          expect(mockFormatEnergy).toHaveBeenCalledWith(
            input.dailyProductionKWh
          );
        else expect(mockFormatEnergy).not.toHaveBeenCalled();
      }
    );
    ...
    test('should handle null API response', async () => {
      ...
      const expectedSpeech =
        "Sorry, I received an empty response from the system. I can't provide a summary right now.";
      const expectedResponse = buildTellResponse(expectedSpeech);
      ...
      expect(mockGcpClient).toHaveBeenCalledTimes(1);
      expect(mockLogger.warn).toHaveBeenCalledWith(
        'Received null or undefined response from getSystemSummary.'
      );
      expect(result).toEqual(expectedResponse);
  ...
  describe('Handling API Call Failures', () => {
    const errorCases = [
      {
        ...
        expectedSpeech:
          "Sorry, I couldn't retrieve the system summary right now due to a connection issue. Please try again later.",
      },
      {
        description: 'timeout error (ETIMEDOUT)',
        error: Object.assign(new Error('Timeout'), { code: 'ETIMEDOUT' }),
        expectedSpeech:
          'Sorry, the request to your solar system timed out. Please try again later.',
      },
      ...
      // --------------------------------------------
      {
        ...
        error: Object.assign(new Error('Server Error'), { statusCode: 500 }),
        expectedSpeech:
          "Sorry, there seems to be an issue with the solar system's reporting service. Please try again later.",
      },
      {
        description: 'server error (503)',
    ...
    test.each(errorCases)(
      'should return specific error message for $description',
      async ({ error, expectedSpeech }) => {
        // Arrange
        ...
        expect(mockLogger.error).toHaveBeenCalledWith(
          { err: error },
          `Error fetching system summary from backend for GetSummaryIntent.`
        );
