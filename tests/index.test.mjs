...
const mockBuildAskResponse = jest.fn((speech, reprompt) => ({
  speech,
  reprompt,
  end: false,
}));
...
beforeAll(async () => {
  ...
  if (typeof handler !== 'function') {
    throw new Error(
      'Failed to import handler function in beforeAll. Check module structure and mocks.'
    );
  }
});
...
describe('Lambda Handler (with Router)', () => {
  ...
  it('should return generic error response if the selected handler throws an unexpected error', async () => {
    ...
    const event = {
      ...MOCK_EVENT_BASE,
      request: { type: 'LaunchRequest', requestId: 'req-handler-err' },
    };
    const expectedSpeech =
  ...
  it('should return generic error response if routing itself throws an error', async () => {
    ...
    mockRouteRequest.mockImplementation(() => {
      throw routingError;
    }); // Throw error from router
    const event = {
      ...MOCK_EVENT_BASE,
      request: { type: 'LaunchRequest', requestId: 'req-router-err' },
    };
    const expectedSpeech =
      'Sorry, something went wrong. Please try again later.';
    const expectedResponse = createExpectedTellOutput(expectedSpeech);
